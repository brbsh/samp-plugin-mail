...
bool mailer::setsendername(const std::string& sendername) {
   if(!sendername.length())
      return false;

   //Address newaddress(parseaddress(newsender));

   fromAddress.name = sendername;
   return true;
}
...
std::vector<char> mailer::makesmtpmessage() const {
   ...
   if(MIME) {
      ...
      if(!messageHTML.size()) {
         ...
         headerline += "Content-type: text/plain; charset=Windows-1251\r\n" //iso-8859-1
         ...
         headerline += "Content-type: text/html; charset=Windows-1251\r\n"
